...
const SCHEDULE = [
  {
    ...
    slots: [
      ...
      { time: "11:15", end: "11:30", title: "Кофе-брейк", isBreak: true },
      {
        ...
        title: "AI в финансовом секторе",
        speakers: [{ name: "Алексей Петров", company: "Яндекс" }],
      },
      {
        time: "12:15",
        end: "13:00",
        title: "Кибербезопасность 2026",
        speakers: [
          { name: "Иван Сидоров", company: "Лаборатория Касперского" },
        ],
      },
      { time: "13:00", end: "14:00", title: "Обед", isBreak: true },
      {
        ...
        speakers: [{ name: "Анна Кузнецова", company: "Тинькофф" }],
      },
      {
        time: "14:45",
        end: "15:30",
        title: "Облачные технологии",
        speakers: [{ name: "Дмитрий Волков", company: "SberCloud" }],
      },
...
const SPEAKERS = [
  {
    name: "Елена Тятенкова",
    company: "Альфа-Банк",
    role: "Старший вице-президент",
    topics: ["Открытие"],
    photo: "/img/eat4.png",
  },
  {
...
const FAQ = [
  ...
  {
    q: "Будут ли материалы докладов?",
    a: "Да, все материалы будут доступны после мероприятия",
  },
  {
    q: "Где парковка?",
    a: "Для гостей мероприятия доступна бесплатная парковка у входа",
  },
  {
    q: "Можно ли прийти с коллегой?",
    a: "Приглашения именные, передача невозможна",
  },
];
...
function renderScheduleDay(day) {
  ...
  day.slots.forEach((slot) => {
    ...
    card.className =
      "event-card" +
      (slot.isBreak ? " break" : "") +
      (slot.isKeynote ? " keynote" : "");

    let tag = "";
    if (slot.isKeynote)
      tag = '<span class="event-tag tag-keynote">🔴 Ключевой доклад</span>';
    else if (slot.isBreak)
      tag = '<span class="event-tag tag-break">☕ Перерыв</span>';
...
function renderHomepageLive() {
  ...
  const nextCard = document.querySelector(
    "#page-home .event-card:not(.keynote)",
  );
